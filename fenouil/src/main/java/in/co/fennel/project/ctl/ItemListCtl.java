 ...
 */
@WebServlet(name = "ItemListCtl", urlPatterns = { "/ctl/itemList" })
public class ItemListCtl extends BaseCtl {
	...
	protected BaseBean populateBean(HttpServletRequest request) {
		log.debug("ItemListCtl populateBean method start");
		ItemBean bean = new ItemBean();
		bean.setName(DataUtility.getString(request.getParameter("name")));
		bean.setCategory(DataUtility.getString(request.getParameter("category")));
		log.debug("ItemListCtl populateBean method end");
		return bean;
	}
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		log.debug("ItemListCtl doGet method start");
		List list = null;
		int pageNo = 1;
		...
		log.debug("ItemListCtl doGet method end");
	}
